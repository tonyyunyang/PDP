...
// Define the maximum length of instruction strings
...
void count_instructions(const char *instruction, InstructionCount *instruction_counts, int *num_instructions) {
   int i;
   for (i = 0; i < *num_instructions; i++) {
      if (strcmp(instruction, instruction_counts[i].instruction) == 0) {
         // Instruction already exists in the list, increment the count
         instruction_counts[i].count++;
         return;
      }
   }

   // Instruction doesn't exist in the list, add it and initialize the count to 1
   strncpy(instruction_counts[i].instruction, instruction, MAX_INSTRUCTION_LENGTH);
   instruction_counts[i].count = 1;

   (*num_instructions)++;
}

InstructionCount instruction_counts[MAX_INSTRUCTIONS];
...
void cycle(State *s, int show_mode)
{
   ...
   if(show_mode) 
   {
      ...
      if(op == 0) 
         ...
         count_instructions(special_string[func], instruction_counts, &num_instructions);
      else if(op == 1) 
         // printf("%8s ", regimm_string[rt]);
         count_instructions(regimm_string[rt], instruction_counts, &num_instructions);
      else {
         count_instructions(opcode_string[op], instruction_counts, &num_instructions);
      //printf("op: %d\n",op);
   ...
   for(;;) 
   {
      ...
      case '3': case 's':
         ...
         for (int i = 0; i < num_instructions; i++) {
            printf("%s: %d\n", instruction_counts[i].instruction, instruction_counts[i].count);
            sum += instruction_counts[i].count;
         }
         printf("Total: %d\n", sum);
         for (int i = 0; i < num_instructions; i++) {
            printf("%s: %f\n", instruction_counts[i].instruction, (float)instruction_counts[i].count / (float)sum);
         }
